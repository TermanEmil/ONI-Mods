...
namespace MoreCanisterFillersMod
{
        ...
        IEffectDescriptor
    {
            ...
            StatesInstance : GameStateMachine<States, StatesInstance, UnfilteredBottleEmptier, object>.GameInstance
        {
            ...
            public StatesInstance(UnfilteredBottleEmptier smi)
                : base(smi)
            {
            ...
            private PrimaryElement GetFirstPrimaryElement()
            {
                ...
                for (var i = 0; i < storage.Count; ++i)
                {
                    ...
                    if (storageGameObject == null) continue;
            ...
            public void Emit(float dt)
            {
                ...
                storage.ConsumeAndGetDisease(consumedTag, amountToConsume, out var diseaseInfo,
                ...
                var idx = element.idx;
                if (element.IsLiquid)
                    FallingWater.instance.AddParticle(cell, idx, amountToConsume, aggregateTemperature, diseaseInfo.idx,
                        diseaseInfo.count, true);
                else
                    SimMessages.ModifyCell(cell, idx, aggregateTemperature, amountToConsume, diseaseInfo.idx,
                        diseaseInfo.count);
            }
        }
        ...
        public class States : GameStateMachine<States, StatesInstance, UnfilteredBottleEmptier>
        {
            ...
            public override void InitializeStates(out BaseState outDefaultState)
            {
                ...
                _statusItem = new StatusItem(nameof(UnfilteredBottleEmptier), string.Empty, string.Empty, string.Empty,
                ...
                Unoperational
                    .TagTransition(GameTags.Operational, Waitingfordelivery)
                    .PlayAnim("off");
                Waitingfordelivery
                    .TagTransition(GameTags.Operational, Unoperational, true)
                    .EventTransition(GameHashes.OnStorageChange, Emptying,
                        smi => !smi.GetComponent<Storage>().IsEmpty())
                    .PlayAnim("on");
                Emptying
                    .TagTransition(GameTags.Operational, Unoperational, true)
                    .EventTransition(GameHashes.OnStorageChange, Waitingfordelivery,
                        smi => smi.GetComponent<Storage>().IsEmpty())
                    .Update("Emit", (smi, dt) => smi.Emit(dt))
